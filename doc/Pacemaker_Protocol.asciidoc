...
The *Length* field contains the number of bytes consumed by the Parameter field. A length of 0 therefore means an Order with no Parameter Bytes.
...
The *Length* field indicates the number of bytes used by the Parameter field in the Order frame and in the Reply frame.
...
This order indicates the operational status of the specified device.
...
.Replies
- OK: The first byte of the Reply Paramter contains indicates one of the following values:

.Device Status values
[width="50%",options="header", cols="30%,70%"]
|=============================
| value | meaning
| 0x0 | device available
| 0x1 | device configuration error
| 0x2 | device fault
|=============================

A status of *device configuration error* indicates that the device is not available fur use because it has missing or invalid device configuration.

A status of *device fault* indicates that the device has sufficent configuration but is unavailable due to a detected hardware fault (e.g., unconnected or shorted sensor).

A status of *device available* indicates that the device has no detected hardware or configuration errors currently.
...
End-stops are always used for homing, but the host can control whether halting on end-stop detection is enabled or disabled for normal movement.
...
This order flushes the order queue. If a queued command is currently executing then it is allowed to continue execution.

The replies to this order contain a count of the executed command blocks since the last Stopped state was cleared. This rolling 16-bit value can be used to determine the state of the system after this command is issued or a Stopped state entered. For the purposes of this count, each movement block counts as one command. All other queued orders also count as 1 command.
...
NOTE: the count of executed command blocks does not include command blocks which failed validation (and therefore not inserted in the queue) or were flushed from the queue before execution but it does include all executed commands regardless of whether or not they were successful.

.Replies
- OK: Queue was cleared successfully (except for any commands currently executing). Bytes 1 & 2 of the Reply parameter indicates the number of move block queue slots are available. Bytes 3 & 4 indicate the number of commands in the queue including any commands currently executing. For this order, the queue count value will only have a value of 0 or 1 depending on whether a command was executing when the Clear Command Block Queue order was received. Bytes 5 & 6 (a rolling 16-bit integer) indicates the number of executed command blocks since the last Stopped state was cleared.
...
The replies to this order contain a count of the executed command blocks since the last Stopped state was cleared. This rolling 16-bit value can be used to determine the state of the system after a stop print is issued or a Stopped state entered. For the purposes of this count, each movement block counts as one command. All other queued orders also count as 1 command.
...
NOTE: the count of executed command blocks does not include command blocks which failed validation (and therefore not inserted in the queue) or were flushed from the queue before execution but it does include all executed commands regardless of whether or not they were successful. The count does include currently executing command blocks.
...
|============================
|Command Block Type Value | Movement Block Type
...
This specifies a 2-byte maximum movement rate value (in steps per second) for each movement axis. This value is used when the axis is selected as the primary movement axis for a linear move command block.

Any axes which are not specified or specified with a movement rate value of 0 will assumed to be unused for movement.

.Parameter
[width="50%",options="header", cols="30%,70%"]
|=============================
| Length In Bytes | Content
| 2 | max steps per second for movement axis 0
| 2 | max steps per second for movement axis 1
| 2 | max steps per second for movement axis 2
| .. | 
| 2 | max steps per second for movement axis N-1
|=============================
...
This order configures the parameters for the underrun avoidance algorithm. This algorithm comes into play when the client believes that a movement queue underrun is likely to occur, that is, the movement queue will be exhausted without a movement checkpoint command being received. 

An example underrun avoidance algorithm is as follows: 

- if the remaining movement time (at nominal speed) is less than the specified minimum period, then the client will decelerate at the specified rate until the movement rate equals the lower of the nominal movement rate or the specified underrun avoidance maximum movement rate
...
As usual, acceleration/deceleration and maximum movement rates are with respect to the currently specified primary axis. A set of maximum movement rates and acceleration values must therefore be specified for each possible axis.

.Parameter
[width="50%",options="header", cols="30%,70%"]
|=============================
| Length In Bytes | Content
| 2 | threshold minimum remaining movement time for underrun avoidance (in 10us units)
| 2 | maximum movement rate (in steps per second) during underrun avoidance for movement axis 0
| 2 | accelation/deceleration rate for underrun avoidance (in steps per second per second) for movement 0
| 2 | maximum movement rate (in steps per second) during underrun avoidance for movement axis 1
| 2 | accelation/deceleration rate for underrun avoidance (in steps per second per second) for movement 1
| 2 | maximum movement rate (in steps per second) during underrun avoidance for movement axis 2
| 2 | accelation/deceleration rate for underrun avoidance (in steps per second per second) for movement 2
| .. | 
| 2 | maximum movement rate (in steps per second) during underrun avoidance for movement axis N-1
| 2 | accelation/deceleration rate for underrun avoidance (in steps per second per second) for movement N-1
|=============================
...
The *primary axis selector* indicates the stepper axis number of the primary axis for this movement command. The speed and acceleration are calculated with respect to the primary axis for the movement block. 
...
The *nominal speed fraction* is the desired movement speed after initial acceleration/deceleration. It is expressed as a fraction of the configured maximum speed for the primary axis (where 0 = 0% and 255 = 100%).
...
This comand block indicates the end of a sequence of moves. The client will not activate the movement underrun prevention algorithms when a Movement Checkpoint command has been received after the last movement command.
