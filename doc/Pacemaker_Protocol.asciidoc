...
All critical communication between the host and the client is started by the host. Only the host may send out Order frames. The client must answer each Order frame with a Reply frame. The timeout for the client to respond by default is 100ms but should be configurable. This includes the time it takes to send and receive the data and is measured on the host. The client therefore has less than 100ms from the time of reception of the order to the sending of the reply.

==== Unsolicited Client Frames
In transport mechanisms which support client initiated data transmission, then the client may at times send out best-effort packets. The two main uses for these unsolicited client frames are for sending debug messages and to notify the host that a new event is available for collection. The Host must not respond to these unsolicited frames. 

An unsolicited client frame has the same structure as a normal Reply frame but has bit 7 in the Control Byte set to 1. The client should increment the sequence number in the Control Byte with each unsolicited frame it sends. These sequence numbers have no correlation to the sequence numbers sent & received in normal Order and Reply frames.

For transport mediums which do not support client initiated communications then the host should either poll for the presence of new events or use a separate interrupt line to indicate this state.
...
If end-stop halting is enabled, then the client should enter the "End-stop hit" Stopped state if an axis is moving but not in homing mode and an enabled endstop for the axis transitions from a not-triggered to triggered state, or remains triggered after the second step.
...
If the axis is moving in homing mode, then if an enabled endstop for the axis triggers (or remains triggered) then the axis will stop but the client will not enter the "End-stop hit" stopped state. 
...
This order configures the end-stop switches which are checked when the specified stepper is moving in either a positive or negative direction. The order also configures whether the end-stop triggers at a high level or low level. The end-stops must be enabled using Enable End-stops before they will cause the end-stop state to affect movement.

All end-stops (including both maximum and minimum) for a single stepper number must specified in the same order. Multiple end-stops can be sepcified for each direction. End-stops are initially disabled and must still be activated using the Activate Endstops command.
...
After the first byte (identifying the target stepper number), the following Parameter data is made up of three byte tuplets which configure the set of max or min end-stops to be checked when the stepper is moving in the specified direction. Multiple steppers can share the same endstop - which is common in a CoreXY configuration.
...
The order Parameter consists of byte pairs which specifies an end-stop device number and whether the end-stop is enabled or disabled.
...
This order gracefully flushes the command block queue. If client is not currently stopped, then the client will attempt to come to a graceful stop at the end of the first movement block where this is possible (for instance by decelerating from the nominal rate to an end-of-block/final speed of zero using the configured underrun avoidance acceleration rate). Once the client is stopped at the end of a movement block (or was staionary to start with) then all remaining items in the command queue are flushed. 
...
Any axes which are specified with a movement rate value of 0 will assumed to be unused. The number of steppers specified in the order should be equal to the number of devices returned by the client in Request Device Count (regardless of whether they are used).
...
The number of steppers specified in the order should be equal to the number of devices advertised by the client (regardless of whether they are used).
...
=== Error/Debug/Warning/Information (0x00-0x03)

The client can use the following Reply Code values to categorizes messages that it sends.
...
The Parameter contains the utf8 information string.

It should be remembered that delivery of these unsolicited frames is not guaranteed and so should not be used as the primary delivery mechanism for serious error information.

=== New Event (0x05)

This optional frame uses a Reply Code of 0x5. The frame has no Parameter content and simply indicates that a new event is available for collection using the Retrieve Events order. The client may choose to transmit another New Event frame if it does not receive a valid Order within a reasonable period. 

[It should be remembered that bit 4 of the normal Reply frame has the same purpose (i.e., to indicate the presence of new events); however, use of the unsolicited frame mechanism alleviates the need for the host to aggresively poll the client for event discovery.]
