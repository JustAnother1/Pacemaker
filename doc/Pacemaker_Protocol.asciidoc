...
| 3 | Inactive Facility? (0 = Active, 1 = Inactive). A value of 1 indicates that the device or facility associated with this element is not currently correctly or completely configured for use. An example of an inactive facility element might be the configuration elements for a heater device where the heater pin or heater control mode has not been configured. Note: this bit is intended to represent a firmware configuration state and not a dynamic or protocol-controlled active/inactive state.
|=============================
...
.OK Reply Parameter Content
[width="50%",options="header", cols="30%,30%"]
|============================
| Length in bytes | Content
| 2 | Number of queue slots available
