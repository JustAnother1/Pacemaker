...
Otherwise, when the host receives a valid frame with matching sequence number and a reply code greater than 0x0f, then retransmission stops and the reply is passed to the application layer. Valid unsolicited client frames received during this period are passed up to the application layer but do not affect order retransmission or reply reception.
...
This unsolicited client frame simply contains a utf8 string with debug-related information. Its primarily intention is to better support development by providing the ability to send non-delayed output messages from the client.
...
However when the movement is performed with the "homing mode" bit set, if any enabled and applicable end-stop triggers, then the matching steppers will stop but the other steppers will continue moving. The client will not enter the "End-stop hit" stopped state.
...
For consistency, the order reply returns the same information as the reply for the Queue Command order. It should be remembered that the reply to the Clear Command Block Queue order is sent immediately and so therefore the Clear Command Block Queue reply will usually indicate that the queue is not empty (unless the queue was already empty when the order arrived). The host should poll the queue count (for instance) using Queue Command order to determine when the queue is empty and the system stopped.
...
The number of steppers specified in the each block of the order should be equal to the number of stepper devices advertised by the client (regardless of whether they are used).
...
The host may retransmit the New Event frame if it does not receive a valid Order frame within a reasonable period. An unsolicited New Event frame should not be generated by the client when a reply is currently outstanding/in-progress for a normal host order frame. 
