...
This order is only allowed if the stepper control has been activated! The order will continue until all axes have homed but the response is sent immediately.

This (non-queued) order will only be accepted when the system is not currently moving the stepper motors or is about to move stepper motors (e.g., the movement queue is empty). 

However, if the queued command extension is supported, then this non-queued Order will actually be queued internally (and will also increase the count of attempted queue commands). This allows the host to monitor the completion of the homing activity by polling the state of the queue and does not require the client to provide two implementations of the homing logic. The command can also be sent using the Queue Command Wrapper in which case the movement queue does not need to be empty before sending.
